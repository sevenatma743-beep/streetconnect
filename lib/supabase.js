...
export async function getPostsPage(cursor = null, currentUserId = null) {
  try {
    let query = supabase
      .from('posts')
      .select(
    ...
    if (error) {
      console.error('Erreur getPostsPage:', error)
      return []
    }
    ...
    console.error('Exception getPostsPage:', err)
    return []
  }
}
...
export async function deletePost(postId, userId) {
  try {
    const { data: post } = await supabase
      .from('posts')
      .select('user_id, media_url, images')
      .eq('id', postId)
      .single()
    ...
    // Supprimer tous les fichiers Storage associés au post
    const allUrls = post.images?.length > 0
      ? post.images
    ...
    for (const rawUrl of allUrls) {
      try {
        ...
        console.error('Erreur parsing URL Storage:', urlErr)
      }
    }
