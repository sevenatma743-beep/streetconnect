...
export async function addComment(commentData) {
  try {
    ...
    await supabase.rpc('increment_comments', { post_id: commentData.post_id })
  ...
  try {
    ...
    return (conversations || []).map(conv => {
      ...
      return {
        ...
        unread: lastMessage && otherMember?.last_read_at 
...
export async function findOrCreateConversation(currentUserId, otherUserId) {
  try {
    // ✅ VALIDATION 1: Vérifier que les IDs existent
    if (!currentUserId || !otherUserId) {
      console.error('❌ findOrCreateConversation: Missing userId', { currentUserId, otherUserId })
      return null
    }

    // ✅ VALIDATION 2: Empêcher conversation avec soi-même
    if (currentUserId === otherUserId) {
    ...
    // ✅ ÉTAPE 1: Chercher conversations de currentUser
    ...
    // ✅ ÉTAPE 2: Si j'ai des conversations, chercher si otherUser est dans l'une d'elles
    ...
    // ✅ ÉTAPE 4: Insérer LES DEUX membres (CRITIQUE - C'ÉTAIT LE BUG!)
...
/**
 * ✅ FONCTION SIMPLIFIÉE - Plus besoin d'auto-add logic
 */
