 ...
 * ✅ Like robuste:
 * - Insert dans likes
 * - PAS de RPC increment (trigger DB / vue / recalcul gère le compteur)
 ...
 * ✅ Unlike robuste:
 * - Delete dans likes
 * - PAS de RPC decrement (trigger DB / vue / recalcul gère le compteur)
 */
...
/**
 * ✅ Follow robuste (sans drift):
 * - Insert dans follows
 * - PAS de RPC increment/decrement (DB triggers gèrent followers_count/following_count)
 * - Si déjà follow => 23505 => ignore
 */
export async function followUser(followingId, followerId) {
  try {
    ...
    if (error) {
      if (error.code === '23505') return true // déjà suivi
