    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          `
          follower_id,
          profiles:profiles!follows_follower_id_fkey(id, username, avatar_url)
        `
        )
        .eq('following_id', u)
    ...
    <div className="min-h-screen bg-street-900">
      <div className="sticky top-0 z-20 bg-street-800/90 backdrop-blur border-b border-street-700">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="text-white font-bold">Abonnés</div>
        </div>
      ...
      <div className="px-4 py-4">
        {loading ? (
          ...
          <div className="text-gray-500 text-center py-10">Aucun abonné</div>
        ) : (
          <div className="space-y-3">
            {filtered.map((r) => {
              ...
              return (
                <button
                  key={r.follower_id}
                  onClick={() => openProfile(r.follower_id)}
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-gray-500 text-xs">{r.follower_id}</div>
                  </div>
                </button>
              )
            })}
